...
mod asset_hub_rococo_tests {
	use super::*;
	use asset_hub_rococo_runtime::{PolkadotXcm, ToWestendOverBridgeHubLegacyXcmRouterInstance};
		...
		>(collator_session_keys(), bridging_to_asset_hub_westend, |bridge_id, is_congested| {
			vec![
				...
				Transact {
					...
					call: RuntimeCall::ToWestendOverBridgeHubLegacyXcmRouter(
		...
		>(
			...
			|| {
				...
				>(
					...
					true,
					|locations, fee| {
						bridge_hub_test_utils::open_bridge_with_extrinsic::<
							Runtime,
							XcmOverAssetHubWestendInstance
	...
	fn westend_xcm_routing_works() {
		...
		ExtBuilder::<Runtime>::default()
			...
			.execute_with(|| {
				...
				let (_, lane_id) = bridge_hub_test_utils::ensure_opened_bridge::<
					Runtime,
					XcmOverAssetHubWestendInstance,
					LocationToAccountId,
					TokenLocation,
				>(
					...
					false,
					|locations, maybe_paid_execution| bridge_hub_test_utils::open_bridge_with_extrinsic::<
						Runtime,
						XcmOverAssetHubWestendInstance,
					>(
						// This should represent `RuntimeOrigin::root()` which represents `Location::here()` for `OpenBridgeOrigin`
