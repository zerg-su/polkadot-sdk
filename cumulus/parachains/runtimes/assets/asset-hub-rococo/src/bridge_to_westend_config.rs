...
/// `WithAssetHubWestendMessagesInstance`.
pub type XcmOverAssetHubWestendInstance = pallet_xcm_bridge_hub::Instance1;
impl pallet_xcm_bridge_hub::Config<XcmOverAssetHubWestendInstance> for Runtime {
	type RuntimeEvent = RuntimeEvent;
...
impl pallet_xcm_bridge_hub_router::Config<ToWestendXcmRouterInstance> for Runtime {
	...
	// We use `UnpaidLocalExporter` with `ViaLocalBridgeHubExporter` ensures that
	...
	type ToBridgeHubSender = pallet_xcm_bridge_hub_router::impls::ViaLocalBridgeHubExporter<
		...
		UnpaidLocalExporter<XcmOverAssetHubWestend, UniversalLocation>,
	>;
